...
repeat(argument0)
{
    roomw = choose(5,7,9,11,13)
    roomh = choose(5,7,9,11,13)
    ...
    if free
    {
        ...
        {
            if (a = 0 && b = 0) && (a = 0 && b = roomh-1) && (b = 0 &&  a = roomw-1) && (a = roomw-1 && b = roomh-1)
            {
    ...
    if prevroomx != 0 {genlink(center.x,center.y,prevroomx,prevroomy)}
    prevroomx = center.x
    prevroomy = center.y
    }
}
if success && debug{sendmsg(string(success)+" out of "+string(argument0)+" rooms successfully generated.")}
if success = 0 {fail += 1} else {fail = 0}
if fail > 10 {exit}
